...
exports.exportReportJSON = exports.analyzeTarget = exports.generateIntelligenceReport = void 0;
...
function calculateRiskScore(findings, entities) {
    let score = 0;
    const sourceCount = new Set(findings.map((f) => { var _a; return f.source || ((_a = f.data) === null || _a === void 0 ? void 0 : _a.platform); })).size;
    ...
    const hasHighRiskSource = findings.some((f) => {
        var _a;
        const source = f.source || ((_a = f.data) === null || _a === void 0 ? void 0 : _a.platform) || '';
...
function generateRiskIndicators(findings, entities) {
    ...
    if (sourceCount > 15) {
        indicators.push({
            ...
            evidence: Array.from(new Set(findings.map((f) => { var _a; return f.source || ((_a = f.data) === null || _a === void 0 ? void 0 : _a.platform); }))).slice(0, 10),
        });
    }
    const uniqueEmails = new Set(findings.map((f) => { var _a; return ((_a = f.data) === null || _a === void 0 ? void 0 : _a.email) || f.email; }).filter(Boolean));
    ...
    if (devSources.length > 0) {
        indicators.push({
            ...
            evidence: devSources.map((f) => { var _a; return f.source || ((_a = f.data) === null || _a === void 0 ? void 0 : _a.platform); }),
        });
    }
    return indicators;
}
// Main controller function
const generateIntelligenceReport = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    try {
        ...
        const targetCase = yield Case_1.default.findById(caseId).lean();
        if (!targetCase) {
            return res.status(404).json({ message: 'Case not found' });
        }
        const findings = yield Finding_1.default.find({ caseId }).lean();
        ...
        findings.forEach((f) => {
            var _a;
            const source = f.source || ((_a = f.data) === null || _a === void 0 ? void 0 : _a.platform) || 'unknown';
            platformDistribution[source] = (platformDistribution[source] || 0) + 1;
        });
        const platformsList = Array.from(new Set(findings.map((f) => { var _a; return f.source || ((_a = f.data) === null || _a === void 0 ? void 0 : _a.platform); }))).map((platform) => {
            var _a;
            return ({
                name: platform || 'unknown',
                status: 'Verified',
                metadata: (_a = findings.find((f) => { var _a; return f.source === platform || ((_a = f.data) === null || _a === void 0 ? void 0 : _a.platform) === platform; })) === null || _a === void 0 ? void 0 : _a.data,
            });
        });
        const keyFindings = [
            {
        ...
        if (highStrengthRelationships.length > 0) {
            ...
            const clusteredPlatforms = new Set();
...
Respond ONLY with a valid JSON object with this exact structure:
{
  ...
  "recommendations": ["string"]
        ...
        try {
            ...
            const aiResponse = JSON.parse(((_b = (_a = chatCompletion.choices[0]) === null || _a === void 0 ? void 0 : _a.message) === null || _b === void 0 ? void 0 : _b.content) || '{}');
            if (aiResponse.overview)
                aiGenerated.overview = aiResponse.overview;
            if (aiResponse.investigationNotes)
                aiGenerated.investigationNotes = aiResponse.investigationNotes;
            if (aiResponse.recommendations && aiResponse.recommendations.length > 0)
                aiGenerated.recommendations = [...aiGenerated.recommendations, ...aiResponse.recommendations];
        }
        catch (e) {
        ...
        yield (0, logActivity_1.logUserActivityDirect)(req.user.id, 'intelligence_report_generated', {
            caseId,
            target: username || email,
...
const analyzeTarget = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { caseId, username, email } = req.body;
        if (!caseId) {
            return res.status(400).json({ message: 'Case ID is required' });
        ...
        const findings = yield Finding_1.default.find(query).lean();
        res.json({
            success: true,
            targetAnalysis: {
                target: { username, email },
                findingsCount: findings.length,
                platforms: Array.from(new Set(findings.map((f) => { var _a; return f.source || ((_a = f.data) === null || _a === void 0 ? void 0 : _a.platform); }))),
...
exports.analyzeTarget = analyzeTarget;
    ...
    try {
        ...
        const report = {
            ...
            executiveSummary: {
                ...
                profileCount: new Set(findings.map((f) => { var _a; return f.source || ((_a = f.data) === null || _a === void 0 ? void 0 : _a.platform); })).size,
                platformsDiscovered: Array.from(new Set(findings.map((f) => { var _a; return f.source || ((_a = f.data) === null || _a === void 0 ? void 0 : _a.platform); }))),
                riskLevel,
            },
            findings,
            entities,
