...
const toCleanLines = (text) => text.split("\n").map((line) => line.trim()).filter(Boolean);
...
const buildSyntheticFindingsFromCase = (caseDoc) => {
  ...
  entities.forEach((entity, idx) => {
    findings.push({
      ...
      email: entity.type === "email" ? entity.value : void 0,
      username: entity.type === "username" ? entity.value : void 0,
      phone: entity.type === "phone" ? entity.value : void 0,
      domain: entity.type === "domain" ? entity.value : void 0,
      data: {
  ...
  if (!findings.length && rawCorpus.trim()) {
    findings.push({
      ...
      source: "Case Dossier",
      confidence: 70,
      data: { raw: rawCorpus.slice(0, 1e3) }
    });
  }
  return findings;
};
...
const buildFallbackMarkdown = (caseDoc, template, findingsCount, riskLevel, visual) => {
  ...
  const entityLines = Object.entries(visual.entitiesByType).map(([type, values]) => `- **${type.toUpperCase()}**: ${values.slice(0, 8).join(", ")}`).join("\n");
  const relationLines = visual.relationshipGraph.edges.slice(0, 12).map((edge) => {
...
const pdfColors = {
  text: (0, import_pdf_lib.rgb)(0.09, 0.12, 0.18),
  muted: (0, import_pdf_lib.rgb)(0.39, 0.45, 0.55),
  border: (0, import_pdf_lib.rgb)(0.88, 0.91, 0.95),
  panel: (0, import_pdf_lib.rgb)(0.97, 0.98, 0.99),
  accent: (0, import_pdf_lib.rgb)(0.06, 0.65, 0.91),
  accentSoft: (0, import_pdf_lib.rgb)(0.91, 0.97, 1),
  white: (0, import_pdf_lib.rgb)(1, 1, 1)
};
const sanitizePdfText = (text) => String(text || "").replace(/→/g, "->").replace(/•/g, "-").replace(/—/g, "-").replace(/–/g, "-").replace(/[“”]/g, '"').replace(/[‘’]/g, "'");
const createPdfWriter = async () => {
  ...
  const smallFont = await pdfDoc.embedFont(import_pdf_lib.StandardFonts.Helvetica);
...
const parseBulletList = (lines) => lines.map((line) => line.replace(/^[-*]\s*/, "").trim()).filter(Boolean).slice(0, 8);
const generateReport = async (req, res) => {
  try {
    ...
    const caseDoc = await import_Case.default.findById(caseId);
    if (!caseDoc) {
    ...
    const { markdownContent, visualReport } = await (0, import_reportService.generateFullReport)(
      caseDoc.get("title"),
      rawFindings,
      targetProfile
    );
    const syntheticDataUsed = !await import_Finding.default.countDocuments({ caseId });
    ...
    const report = new import_Report.default({
      caseId,
      template,
    ...
    await report.save();
    await import_Case.default.findByIdAndUpdate(caseId, {
      reportGenerated: true,
      reportTemplate: template,
      lastReportId: report._id
    });
    await (0, import_logActivity.logUserActivity)(req, "intelligence_report_generated", `Generated ${template} report for case: ${caseDoc.get("title")}`, caseId);
...
const getReport = async (req, res) => {
  try {
    ...
    const report = await import_Report.default.findById(reportId).populate("caseId", "title description");
    if (!report) {
...
const getCaseReports = async (req, res) => {
  try {
    ...
    const reports = await import_Report.default.find({ caseId }).sort({ createdAt: -1 }).limit(10).lean();
...
const exportReportPDF = async (req, res) => {
  try {
    const { reportId } = req.params;
    const report = await import_Report.default.findById(reportId).populate("caseId", "title description category priority status clues targetProfile");
    if (!report) {
      return res.status(404).json({ message: "Report not found" });
    }
    ...
    const riskColor = report.riskLevel === "Critical" ? (0, import_pdf_lib.rgb)(0.83, 0.15, 0.15) : report.riskLevel === "High" ? (0, import_pdf_lib.rgb)(0.92, 0.38, 0.1) : report.riskLevel === "Medium" ? (0, import_pdf_lib.rgb)(0.79, 0.62, 0) : (0, import_pdf_lib.rgb)(0.09, 0.64, 0.28);
...
const exportReportJSON = async (req, res) => {
  try {
    const { reportId } = req.params;
    const report = await import_Report.default.findById(reportId).populate("caseId");
    if (!report) {
      return res.status(404).json({ message: "Report not found" });
    }
