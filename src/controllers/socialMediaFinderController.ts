 ...
 */
const extractUsernamesFromEmail = (email: string): string[] => {
...
/**
 * Comprehensive platform detection with multiple methods
 */
 ...
 */
const checkPlatform = async (username: string, platform: any): Promise<SocialProfile | null> => {
  try {
 ...
 */
export const findSocialMediaProfiles = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const foundProfiles: SocialProfile[] = [];
    ...
    results.byCategory = {
      social_networks: uniqueProfiles.filter(p => 
        ['Twitter', 'Instagram', 'Facebook', 'TikTok', 'LinkedIn', 'Snapchat', 'Pinterest', 'Reddit', 'Mastodon', 'Bluesky', 'Threads'].includes(p.platform)
      ),
      developer: uniqueProfiles.filter(p => 
        ['GitHub', 'GitLab', 'Bitbucket', 'Stack Overflow', 'CodePen', 'Repl.it', 'Glitch'].includes(p.platform)
      ),
      content: uniqueProfiles.filter(p => 
        ['YouTube', 'Twitch', 'Medium', 'Substack', 'Rumble', 'Dailymotion'].includes(p.platform)
      ),
      gaming: uniqueProfiles.filter(p => 
        ['Steam', 'Discord', 'Xbox Live', 'PlayStation', 'Twitch'].includes(p.platform)
      ),
      other: uniqueProfiles.filter(p => 
        !['Twitter', 'Instagram', 'Facebook', 'TikTok', 'LinkedIn', 'Snapchat', 'Pinterest', 'Reddit', 'Mastodon', 'Bluesky', 'Threads',
    ...
    // Log activity
    logUserActivity(req, 'social_media_finder', 'Social Media Profile Finder', { 
      input, 
 ...
 */
export const quickSocialMediaLookup = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const quickResults: SocialProfile[] = [];
