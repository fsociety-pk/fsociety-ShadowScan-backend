...
function extractDomains(text: string): string[] {
  ...
  // exclude emails — already captured separately
  const emails = extractEmails(text);
  return all.filter((d) => !emails.some((e) => e.endsWith(d)));
}

function extractNames(text: string): string[] {
  // Simple heuristic: Title Case 2-word sequences that are not known stopwords
  const stopWords = new Set(['The', 'This', 'That', 'With', 'From', 'Into', 'Over', 'Under', 'Between',
    'After', 'Before', 'During', 'About', 'Against', 'Along', 'Among', 'Also', 'While',
    'When', 'Where', 'Which', 'What', 'Have', 'Been', 'Being', 'Just', 'More', 'Than',
    'Such', 'Both', 'Each', 'Even', 'Only', 'Very', 'Much', 'Many', 'Some', 'There',
    'They', 'Their', 'Your', 'Email', 'Phone', 'User', 'Name', 'Date', 'Born', 'Case',
    'Risk', 'High', 'Medium', 'Located', 'Lives', 'Work', 'Known', 'Based', 'Social',
    'Account', 'Platform', 'Target', 'Subject', 'Found', 'Data', 'Info', 'Full',
  ]);
...
function extractOrganizations(text: string): string[] {
  ...
  // Also detect capitalized words followed by Inc., Ltd., Corp., LLC
...
function detectPlatforms(text: string): string[] {
  const platformMap: Record<string, string> = {
    ...
    github: 'GitHub', gitlab: 'GitLab', youtube: 'YouTube', tiktok: 'TikTok',
    reddit: 'Reddit', telegram: 'Telegram', whatsapp: 'WhatsApp', discord: 'Discord',
    snapchat: 'Snapchat', pinterest: 'Pinterest', tumblr: 'Tumblr', medium: 'Medium',
    stackoverflow: 'Stack Overflow', pastebin: 'Pastebin', twitch: 'Twitch',
    spotify: 'Spotify', skype: 'Skype', signal: 'Signal', viber: 'Viber',
  };
...
function computeRisk(entities: ReturnType<typeof buildEntityMap>): {
  score: number;
  level: 'Low' | 'Medium' | 'High';
  indicators: { category: string; severity: 'Low' | 'Medium' | 'High'; description: string; evidence: string[] }[];
} {
  let score = 0;
  const indicators: { category: string; severity: 'Low' | 'Medium' | 'High'; description: string; evidence: string[] }[] = [];
...
function buildEntityMap(text: string) {
  return {
...
function detectRelationships(entities: ReturnType<typeof buildEntityMap>): {
  entity1: string; entity2: string; relation: string; strength: 'Strong' | 'Moderate' | 'Weak';
}[] {
  const rels: { entity1: string; entity2: string; relation: string; strength: 'Strong' | 'Moderate' | 'Weak' }[] = [];
...
function deriveKeyFindings(entities: ReturnType<typeof buildEntityMap>, risk: { level: string; score: number }): { finding: string; confidence: number; category: string }[] {
  const findings: { finding: string; confidence: number; category: string }[] = [];
  ...
  if (entities.phones.length > 0)
    findings.push({ finding: `${entities.phones.length} phone number(s) extracted — potential SIM-swap vulnerability`, confidence: 0.90, category: 'Personal Data' });
  ...
  if (entities.organizations.length > 0)
    findings.push({ finding: `Organizational affiliation detected: ${entities.organizations[0]}`, confidence: 0.80, category: 'Organization' });
  ...
  findings.push({ finding: `Overall risk assessment: ${risk.level} (Score: ${risk.score}/100)`, confidence: 1.0, category: 'Risk Assessment' });
  return findings;
}

// ─── Recommendations ──────────────────────────────────────────────────────────

function generateRecommendations(entities: ReturnType<typeof buildEntityMap>, riskLevel: string): string[] {
  const recs: string[] = [];
  if (entities.emails.length > 0) {
...
export const analyzeOsintData = async (req: Request, res: Response) => {
  try {
    const { targetData, targetLabel } = req.body as { targetData: string; targetLabel?: string };
    ...
    // Primary target name — best guess
    ...
    const report = {
      ...
      relationshipAnalysis: {
        ...
        clusterNotes: relationships.length > 0
          ? `Analysis identified ${relationships.filter((r) => r.strength === 'Strong').length} strong and ${relationships.filter((r) => r.strength === 'Moderate').length} moderate connection(s) between discovered entities.`
          : 'Insufficient entity co-occurrence to establish confirmed cross-platform relationships.',
      },
      riskAssessment: {
    ...
    res.status(500).json({ message: 'Analysis engine encountered an error.', error: (error as any).message });
  }
};
