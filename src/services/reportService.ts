...
export interface GeminiReportResult {
    markdownContent: string;
    visualReport: VisualReportFromAI;
}
...
const getColor = (type: string) => TYPE_COLORS[type.toLowerCase()] || '#64748b';
...
const buildPrompt = (caseTitle: string, rawFindings: string, targetProfile: any): string => `
...
const buildFallbackVisual = (caseTitle: string, rawFindings: string, targetProfile: any): VisualReportFromAI => {
    ...
    const edges: { source: string; target: string; relation: string; strength: 'weak' | 'medium' | 'strong' }[] = [];
    const emails: string[] = [];
    ...
    lines.forEach((line, idx) => {
        ...
        let type = 'note';
        if (['email'].includes(key)) { type = 'email'; emails.push(value); entitiesByType['Email Addresses'] = [...(entitiesByType['Email Addresses'] || []), value]; }
        else if (['phone', 'contact', 'mobile'].includes(key)) { type = 'phone'; phoneNumbers.push(value); entitiesByType['Phone Numbers'] = [...(entitiesByType['Phone Numbers'] || []), value]; }
        else if (['username', 'user', 'handle'].includes(key)) { type = 'username'; usernames.push(value); entitiesByType['Usernames'] = [...(entitiesByType['Usernames'] || []), value]; }
        else if (['ip'].includes(key)) { type = 'ip'; ipAddresses.push(value); entitiesByType['IP Addresses'] = [...(entitiesByType['IP Addresses'] || []), value]; }
        else if (['domain'].includes(key)) { type = 'domain'; domains.push(value); entitiesByType['Domains'] = [...(entitiesByType['Domains'] || []), value]; }
        else if (['name', 'alias', 'friend', 'known_associate'].includes(key)) { type = 'person'; entitiesByType['Names / Aliases'] = [...(entitiesByType['Names / Aliases'] || []), value]; }
        else if (['location', 'city', 'address'].includes(key)) { type = 'location'; entitiesByType['Locations'] = [...(entitiesByType['Locations'] || []), value]; }
        else if (['organization', 'employer', 'company'].includes(key)) { type = 'organization'; entitiesByType['Organizations'] = [...(entitiesByType['Organizations'] || []), value]; }
        else if (['telegram', 'discord', 'twitter', 'x_profile', 'linkedin', 'github'].includes(key)) { type = 'social'; socialAccounts.push(value); entitiesByType['Social Media Accounts'] = [...(entitiesByType['Social Media Accounts'] || []), `${key}: ${value}`]; }
        else if (['wallet'].includes(key)) { type = 'wallet'; wallets.push(value); entitiesByType['Crypto Wallets'] = [...(entitiesByType['Crypto Wallets'] || []), value]; }
        else if (['vehicle'].includes(key)) { entitiesByType['Vehicles'] = [...(entitiesByType['Vehicles'] || []), value]; }
